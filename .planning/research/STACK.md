...
**Domain:** Cross-platform pub quiz app (iOS, Android, Web)
...
**Architecture note:** Zustand owns client state (current quiz, selected answers, UI toggles). TanStack Query owns server state (fetched questions, categories, user stats). They complement, never overlap.
...
**Architecture note:** Two storage layers is intentional. MMKV for fast key-value access (settings, tokens). SQLite for structured question data that needs querying. Do not try to force one tool to do both jobs.
...
**What about Moti?** Moti (v0.30.0) has not been updated in over 12 months and shows signs of abandonment. Do NOT adopt it. Use Reanimated directly -- the API in v4 is clean enough, and you avoid depending on an unmaintained wrapper. For simple enter/exit animations, Reanimated's `entering` and `exiting` layout animations cover 90% of what Moti offered.
...
**Warning:** As of April 2026, the Claude Code Scheduled Triggers API has reported HTTP 500 errors. This is flagged as a known issue. Have a fallback plan: agents can also run via local Claude Code sessions with `/loop` scheduling, or via simple cron jobs calling the Claude API directly. Do not make the entire pipeline dependent on cloud scheduling being stable at launch.
...
```bash
# Create project
npx create-expo-app@latest pub-quiz --template tabs

# Core dependencies
npx expo install expo-router expo-sqlite expo-font expo-secure-store expo-haptics expo-image expo-splash-screen expo-constants react-native-reanimated
...
# Dev dependencies
npm install -D @types/react supabase
```
...
**If multiplayer needs sub-50ms latency (competitive real-time):**
...
**If the question database exceeds 100K questions:**
...
**If going offline-first (no connectivity at all):**
...
**If monetization is subscription-based:**
...
The PROJECT.md specifies "React Context + TanStack Query for server state." This research challenges the React Context portion:

**Problem with Context for quiz state:**
...
**Recommendation:** Use Zustand for client state, TanStack Query for server state. Keep React Context only for theme/config injection (where it belongs).
...
---
*Stack research for: Cross-platform pub quiz app*
*Researched: 2026-04-04*
